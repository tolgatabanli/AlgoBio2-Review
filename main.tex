...
\rhead{Kompiliert am \today\ um \currenttime}
\pagestyle{fancy}
\begin{document}
\begin{multicols}{2}
...
\begin{itemize}
    ...
    \[ \Gamma_A(x,y) \leq \frac{\text{Elementanzahl}}{\frac{\text{Elementenzahl}}{r}}=r \]
\end{itemize}
...
\begin{enumerate}
    \setcounter{enumi}{4}
    \item Zeige, dass die Lösung von B mithilfe von $g()$ zu einer zulässigen Lösung für A abgebildet wird. Benutze hierfür die beschriebene $f-g$-Beziehung.
\end{enumerate}

\subsection*{Approximationsgüte}
\begin{enumerate}
    \setcounter{enumi}{5}
    \item Zeige: \[ \Gamma_B(f(x, \epsilon), y) \le 1 + \alpha(\epsilon) \implies \Gamma_A(x, g(x, y, \epsilon)) \le 1 + \epsilon \]
    \begin{enumerate}
        \item Wissen: Güte für eine Lösung von B $\leq 1+\alpha(\epsilon)=1+\epsilon$
        \item Benutze $g(x,y,\epsilon)^\ast$ um zu zeigen: $\frac{\text{A-Lösung}}{\text{A-Optimum}}\overset{\ast}{=}\frac{\text{B-Lösung}}{\text{B-Optimum}}\overset{\substack{\text{wie}\\\text{oben}}}{=}1+\epsilon$
    \end{enumerate}
\end{enumerate}
...
Zeige, dass eine Zufallsvariablenfolge Markov-Kette ist: \[\text{Ws}[X_n=q_n\mid X_{n-1}=q_{n-1}] = \dots = \] \[\text{Ws}[X_n=q_n\mid (X_{n-1},\dots,X_1)=(q_{n-1},\dots,q_1)]\] \\ \\
