...
\begin{enumerate}
    \item Sei $S$ eine Lösung für $f(x,\epsilon)$ und $S^\ast$ die optimale.
    \item Dann gilt
    \[\frac{\text{Maß für }f(x, \epsilon)}{\text{Opt. für }f(x, \epsilon)}=\Gamma_{\mu'}(f(x, \epsilon),S)\leq 1 + \alpha(\epsilon)=1+\epsilon\]
    \item \[\frac{\text{Maß für x}}{\text{Optimum für x}}\leq\frac{\mu(B)}{\mu(B^\ast)}=1+\epsilon\]
\end{enumerate}
