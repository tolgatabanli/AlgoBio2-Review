...
\begin{enumerate}
    \item Sei $B$ eine Lösung für $f(x,\epsilon)$.
    \item Sei $B^\ast$ die optimale Lösung für $f(x, \epsilon)$
    \item Dann gilt
    \[\frac{\mu(B)}{\mu(B^\ast)}=\Gamma_{\mu'}(f(x, \epsilon),B)\leq 1 + \alpha(\epsilon)=1+\epsilon\]
